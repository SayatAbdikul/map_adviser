...
4. Сгенерировать ровно 3 варианта маршрута. Варианты должны отличаться выбором конкретных заведений (например, "Ближайший банк А" vs "Банк Б чуть дальше, но с лучшим рейтингом") или порядком посещения, чтобы предложить пользователю выбор. Все маршруты должны стремиться к минимальному общему расстоянию.
...
async def plan_route(
    ...
    Args:
        ...
        optimize: Optimization criteria

    Returns:
        Route response dictionary
    """
    mode_ru = "пешком" if mode == "walking" else "на машине"
    optimize_ru = "по расстоянию" if optimize == "distance" else "по времени"
